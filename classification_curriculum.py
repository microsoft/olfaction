...
def main(args, exp_config, train_set, val_set, test_set):
    ...
    if args['pretrain']:
        ...
        ## TODO: Needs more permenant fix - right now manually re-setting predictor head to account for added features
        ## from ESM embeddings modifying shape of predictor input when loading model weights, this is later
        ## reset.
        #model.predict = MLPPredictor(2 * model.gnn.hidden_feats[-1] + 1280, exp_config['predictor_hidden_feats'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('-prev', '--prev_data_n_tasks', type=int, default=574,
                        help= "For curriculum learning, specify n_tasks of previous dataset to correctly load saved model.")
    parser.add_argument('-pmp', '--prev_model_path', type=str, default='M2OR_Uniprot_original_GCN',
                        help = 'For curriculum learning, specify path to trained model to correctly load model.')
